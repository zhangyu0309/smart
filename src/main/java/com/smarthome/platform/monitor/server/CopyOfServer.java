	...
	private byte[] MessageHandle(byte[] msg) {
		...
		if (commands.length > 1){
			for (int i = 0; i < commands.length; i++) {
				MessageHandle(commands[i].getBytes());
			}
			return null;
		}
...
//			CURRVALUE:3A00010200000000000000000000000000003923.
			...
			if (message.endsWith(".")){
				...
				//结尾有零碎数据
				MessageHandle(message.substring(0, message.lastIndexOf(".") + 1).getBytes());
			...
			if (tempdata.toString().startsWith("CURRVALUE:") && 
				...
				if (tempdata.toString().endsWith(".")){
					logger.info("-->full");
					MessageHandle(tempdata.toString().getBytes());
					tempdata = new StringBuilder("");
				}else {
					if (tempdata.toString().contains(".")){
						logger.info("-->full with half");
						MessageHandle(tempdata.toString().substring(0, tempdata.toString().lastIndexOf(".") + 1).getBytes());
